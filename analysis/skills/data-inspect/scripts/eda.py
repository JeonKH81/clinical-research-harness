...
def map_variables(prereg, columns):
    ...
    # 생존분석용 time-to-event 변수 자동 탐지 (Cox/KM에 필요).
    # 이 매핑이 없으면 Phase 5의 Cox 분석이 실패한다. 우선순위 토큰으로 휴리스틱 탐지.
    outcome_col = mapping.get("outcome_primary")
    time_match = None
    for pref in ("time_to", "tte", "duration", "followup", "follow_up",
                 "survival", "days", "months", "years", "time"):
        for cl, c in cols_lower.items():
            if pref in cl and c != outcome_col:
                time_match = c
                break
        if time_match:
            break
    mapping["time_to_outcome"] = time_match  # 휴리스틱 — 사용자 확인 필요
