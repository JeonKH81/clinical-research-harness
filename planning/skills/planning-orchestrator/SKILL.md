...
description: 임상연구 "계획 하네스"의 진입점. Phase 0–3 (Intake → 문헌검색 → 가설 PICO 정제·사전등록 → IRB 연구계획서) 라우팅과 Human-in-the-loop 게이트 G0–G3를 강제한다. "임상연구 시작", "이 주제로 연구하고 싶다", "문헌 찾아줘", "가설 정리", "연구계획서", "IRB", 그리고 "다시", "이어서", "재실행", "수정", "업데이트", "이전 결과 기반" 같은 후속·재작업 요청을 포함한 자연어에서 자동 호출. 데이터 분석·논문은 별도 clinical-research-analysis 플러그인이 담당.
license: MIT
---
...
[2] 키워드 분류
  ...
  ├─ "연구계획서", "IRB", "프로토콜"                       → Phase 3 (protocol-writer)
...
### 한계 명시
- 기관 IRB 시스템 자동 연동 없음 — 사용자 응답을 그대로 신뢰
...
**정상 흐름 (happy path):**
1. "심장 연구 시작하고 싶다" → Phase 0 진입, 5항목(프로젝트명·주제·환자군·데이터 위치·기존 가설) 수집
2. G0 3항목(IRB 범위·PHI 정책·학술 책임) 모두 "예" → Phase 1 진입
3. literature-scout가 9개 연구 기회 후보 제시 → G1에서 사용자가 선택(예: "B1")
4. Phase 2 가설 PICO 정제 → G2 사전등록(prereg.json + 해시) 기록
5. Phase 3 연구계획서(.docx) 생성 → G3 IRB 상태 입력 → 분석 하네스로 핸드오프 안내

**에러 흐름 (반드시 이렇게 동작해야 함):**
- **G0 거부**: "PHI 차단 정책 동의" 항목에 "아니오" → Phase 1 진입 **차단**, 정책 안내, evolution_log에 기록
- **G0 우회 시도**: "그냥 문헌 검색부터 해줘" → 거절, Phase 0으로 환원, 시도 사실 기록
- **같은 프로젝트명 재호출**: 기존 폴더 인식 → **덮어쓰지 않고** 어느 Phase부터 이어갈지 안내
- **범위 밖 요청**: "Cox 분석 돌려줘" / "논문 초안 써줘" → 이 플러그인 범위 밖 안내 + `clinical-research-analysis` 플러그인 사용 권고
- **근거 없는 가설**: PMID/DOI 근거가 전혀 없는 연구 기회 → 거절(Citation Grounding 비타협)
