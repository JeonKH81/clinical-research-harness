---
name: prereg-lock
description: 임상연구 가설과 분석 계획을 SHA-256 해시로 기록하고 변경 사실을 자동 로깅한다 (Soft 기록 모델, informed-consent 기반). HARKing 방지를 위한 자동 감사 트레일 역할. 계획 하네스 Phase 2의 G2 게이트에서 hypothesis-refiner agent가 호출. 분석 하네스는 본 스킬에 의존하지 않고 자체 prereg_check.py로 무결성을 검증한다.
license: MIT
---
...
- 무결성 검증(verify 명령)은 계획 하네스 각 Phase 진입 시 사용. (분석 하네스는 본 스킬 대신 자체 prereg_check.py 사용 — 플러그인 독립성)
...
4. 파일 권한은 일반 유지 (chmod 444 강제 안 함 — Soft 기록 모델). 변경은 차단되지 않고 verify 시 드리프트로 감지됨
5. evolution_log에 기록 사실 기록 (PREREG_RECORDED)
...
계획 하네스의 후속 Phase 진입 시 호출. (분석 하네스는 자체 prereg_check.py로 검증 — 본 스킬 비의존)
