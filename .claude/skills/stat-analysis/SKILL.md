...
```
[1] prereg.json 무결성 검증 (lock.py verify)
    드리프트 → 경고 + 사용자 인지 확인 (Soft 모델)
        ↓
[2] data_file_hash 검증 (Phase 4에서 기록된 것과 비교)
    불일치 → 경고 + informed-consent
        ↓
[3] IRB 게이트 점검 (irb_status)
    approved/exempt/expedited → 진행
    submitted/pending → 경고 + override 요구 (override 시 evolution_log 기록)
        ↓
[4] Table 1 자동 생성 (baseline characteristics + SMD)
        ↓
[5] 1차 분석 (사전등록된 primary endpoint, primary_method 그대로)
        ↓
[6] 사전등록 부수 분석 (secondary outcomes)
        ↓
[7] 민감도 분석 (사전등록된 것: complete-case, MI, IPTW, PSM 등)
        ↓
[8] 진단 플롯 (PH, calibration, residuals, ROC 등)
    가정 위반 시 자동 경고 + 대안 모델 제안
        ↓
[9] Exploratory 분석 (있다면 별도 섹션, BH-FDR 자동 적용)
        ↓
[10] STROBE 22항목 자동 점검 + G5 게이트
```
