...
def verify_prereg(prereg_path, project):
    """prereg-lock의 verify 호출."""
    lock_script = Path(__file__).resolve().parents[2] / "prereg-lock" / "scripts" / "lock.py"
    try:
        result = subprocess.run(
            [sys.executable, str(lock_script), "verify", "--project", project],
            capture_output=True, text=True, timeout=30
        )
        if result.returncode != 0:
            print("[FATAL] Pre-registration integrity verification FAILED.", file=sys.stderr)
            print(result.stderr, file=sys.stderr)
            sys.exit(2)
        print("[OK] Pre-registration verified.")
    except FileNotFoundError:
        print(f"[WARN] lock.py not found at {lock_script}; skipping verification (NOT for production)",
              file=sys.stderr)
