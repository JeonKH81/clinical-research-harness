...
## 설치 B — zip 단독 (오프라인 / GitHub 불필요)

각 플러그인 디렉토리는 자체 `.claude-plugin/marketplace.json`(`source: "."`)을 포함하므로, **zip 하나만 풀면 그 폴더가 곧 설치 가능한 마켓플레이스**입니다. GitHub 접속이나 repo 클론이 필요 없습니다.

```bash
# 1) 압축 해제
unzip clinical-research-planning.zip
unzip clinical-research-analysis.zip

# 2) 풀린 폴더를 로컬 마켓플레이스로 등록 (절대경로 권장)
/plugin marketplace add ./clinical-research-planning
/plugin marketplace add ./clinical-research-analysis

# 3) 설치 — install 형식은 plugin@marketplace
/plugin install clinical-research-planning@clinical-research-planning
/plugin install clinical-research-analysis@clinical-research-analysis
```

> 두 방식은 동일한 플러그인을 설치합니다. 한 가지만 쓰세요(중복 등록 시 같은 플러그인이 두 마켓플레이스에 보일 수 있음).
...
│
├── planning/                           # 하네스 1 (플러그인 + zip 단독 마켓플레이스)
│   ├── .claude-plugin/plugin.json
│   ├── .claude-plugin/marketplace.json # zip 단독 설치용 (source: ".")
│   ├── skills/
...
│       └── protocol-writer.md
│
├── analysis/                           # 하네스 2 (플러그인 + zip 단독 마켓플레이스)
│   ├── .claude-plugin/plugin.json
│   ├── .claude-plugin/marketplace.json # zip 단독 설치용 (source: ".")
│   ├── skills/
